...
Download and install Python [3.10](https://www.python.org/downloads/release/python-31011/) (recommended) or 3.9, if needed. If this is your first installation of Python on your workstation, check the "Add Python to PATH" box during echo installation.

## Basic Users

These instructions assume no working knowledge of command line interface or GitHub.

Download the [FATES-BLH-ScaleAgeing repository](https://github.com/SEFSC/FATES-BLH-ScaleAgeing) from GitHUb: click the green "<> Code" button and select "Download ZIP". Extract to your desired local location.

To use the Segment Anything Model (SAM) for image processing, [download this model](#download-segmentation-model) as well. Otherwise, skip this step and use simple binary segmentation.
...
*This is optional for users who prefer Anaconda over pip.*
...
The model code and most of its dependencies are [available on GitHub]({{< meta website.repo-url >}}). To retrieve it, clone the `{{< meta repo.prod-branch >}}` branch of this repo, which is currently functioning as the production version of the model:
...
2. Clone the repository into the desired directory
   ...
   ```bash
   git clone -b {{< meta repo.prod-branch >}} {{< meta website.repo-url >}} .
   ```
...
This model has the option of using a [Segment Anything Model](https://arxiv.org/abs/2408.00714) (SAM) to find the scale within each image. To use this method, one must [download a model checkpoint](https://github.com/facebookresearch/segment-anything?tab=readme-ov-file#model-checkpoints). The [ViT-B model](https://dl.fbaipublicfiles.com/segment_anything/sam_vit_b_01ec64.pth) is recommended. For convenience, consider saving this file in the same directory as model script, for example `~/{{< meta repo.name >}}/scripts/weights`.

::: {.callout-important}
You only need to download this if you intend to use Segment Anything Model method to image preprocessing. See [Using the Model](usage.qmd#configuration-file) for more information. 
:::

### Create a Virtual Environment

Virtual environments are used to manage Python packages. Options exist for either pip or conda environments. Choose one.

### Option 1: pip

Create and activate a new pip virtual environment and install package dependencies using the `requirements.txt` file packaged with the repo:

```bash
python -m venv .scaleageing
...
Create a virtual environment specifically for this model. In a terminal window, navigate (`cd`) into the directory containing the `environment.yml` file. This should be the parent directory of the cloned repository. For example,

```bash
cd "c:/Users/user.name/Documents/ageing/{{< meta repo.name >}}"
```
...
::: {.callout-tip title="Deleting a virtual environment"}
In the unlikely event you need to delete this virtual environment, such as to troubleshoot or to restart from a clean slate, use the following:

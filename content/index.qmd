...
5. The new square image containing just the scale of interest is passed to a trained custom [residual neural network](https://en.wikipedia.org/wiki/Residual_neural_network) (resnet) deep learning classification model along with a [metadata CSV file](metadata.qmd) described later. Model output is saved to a CSV file.

Implementation instructions follow. Be sure to [set up and configure a Python environment](setup.qmd) before the first use.  
...
<br>
For more information, including an execution example, see [Using the Model](usage.qmd) in the docs.
...
See [Getting Started](setup.qmd) for installation and setup instructions.

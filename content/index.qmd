...
```bash
python process-images.py -c <config_dir>
```

where `<config_dir>` is the path to the configuration file containing model settings described [below](#options).

::: {.callout-tip}
In the command snippets above, `-c` is an abbreviation for the `--config_path` flag for convenience. For clarity, the full flag `--config_path` is used throughout these docs, but either option can be used for any of the commands that follow.
:::

The ageing model itself is wrapped inside a second Python script called `predict-ages-multimodal.py`. To execute, run the following command in a command line terminal:
...
<br>
For more information, including an execution example, see [Using the Model](/content/usage.qmd) in the docs.
...
| Key                       | Description                                     |
|---------------------------|-------------------------------------------------|
| `raw_image_path`          | Path to raw images. Best to include the full path in quotations. Example: "G:/Shared drives/NMFS SEFSC FATES Advanced Technology/BIOLOGY_LIFE_HISTORY_DATA/2020_Plant_10/Raw_Images" |
...
| Key                       | Description                                     |
|---------------------------|-------------------------------------------------|
| `image_path`              | Path to preprocessed image folder. Best to include the full path in quotations. This should generally match `preprocessed_image_path` from the pre-processing step unless you want to age a different set of cropped images. |
| `metadata_path`         | Path to a single CSV file containing the metadata for all images in `image_path` (directory and metadata file name). Best to include the full path in quotations. |
...
<details>
  ...
  <ul>
    <li> <b>2025.0.3 (BETA)</b> (September 2025):
      <ul>

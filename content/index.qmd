...
where `<config_dir>` is the path to the configuration file containing model settings described [below](#options). The ageing model itself is wrapped inside a second Python script called `Scale_Aging_Inference_Script_Multimodal.py`. To execute, run the following command in a command line terminal:

```bash
python Scale_Aging_Inference_Script_Multimodal.py --config_path <config_dir>
```

or

```bash
python Scale_Aging_Inference_Script_Multimodal.py -c <config_dir>
```
